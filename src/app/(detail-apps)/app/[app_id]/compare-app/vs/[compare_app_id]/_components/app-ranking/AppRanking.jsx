...
import { DoubleRightOutlined, DownOutlined, ExclamationCircleOutlined, UpOutlined } from '@ant-design/icons';
import MyLink from '@/components/ui/link/MyLink';

export default function AppRanking({ compareAppData, initialDataTopKW }) {
  ...
  const topKeywords = initialDataTopKW;
  ...
  const uniqueMostPopularPositionCollections = Array.from(
    new Map(
      combinedMostPopularPositionCollections
        ...
        .map((collections_popular) => [collections_popular.collection_name, collections_popular]), // Map to [collection, collections_popular]
    ).values(),
  );
  ...
  const transposedDataRanking = [
    ...
    },
    {
      ...
      values: [compareAppData[0]?.app_host || {}, ...dataAppCompare].map((appItem = {}, index) => {
        ...
        // const mostPopularPositionCollectionToShow1 = keywordRanks.slice(0, maxItemsToShow);
