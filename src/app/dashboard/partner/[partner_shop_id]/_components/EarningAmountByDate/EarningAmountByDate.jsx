...
export default function EarningAmountByDate({ dataPartner }) {
  ...
  const processEarningData = () => {
    ...
    updatedDataPartner.forEach((app) => {
      app.earning_amount_by_date?.forEach((entry) => {
        allDates.add(entry._id);
      });
    });
    ...
    const chartData = sortedDates.map((date) => {
      ...
      updatedDataPartner.forEach((app) => {
        const appEarning = app.earning_amount_by_date?.find((entry) => entry._id === date);
        dataPoint[app.app_name] = appEarning && appEarning.amount >= 0 ? appEarning.amount : 0;
      });
